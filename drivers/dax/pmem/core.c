...
struct dev_dax *__dax_pmem_probe(struct device *dev, enum dev_dax_subsys subsys)
{
	...
	if (rc)
		return ERR_PTR(rc);
	rc = nvdimm_setup_pfn(nd_pfn, &pgmap, &mi);
	if (rc)
		return ERR_PTR(rc);
	devm_nsio_disable(dev, nsio);

	/* reserve the metadata area, device-dax will reserve the data */
	if (!devm_request_mem_region(dev, nsio->res.start, mi.map_offset
				+ mi.map_pad, dev_name(&ndns->dev))) {
	...
	dax_region = alloc_dax_region(dev, region_id, &res,
			...
			le32_to_cpu(nd_pfn->pfn_sb->align), PFN_DEV|PFN_MAP);
	if (!dax_region)
